...
public class RecyclerViewAdapterPokemons extends RecyclerView.Adapter<RecyclerViewAdapterPokemons.ViewHolder>{

    private ArrayList<Pokemons> mData;
    private LayoutInflater mInflater;
    private RecyclerViewAdapterTiposPokemon.ItemClickListener mClickListener;
    private Context context;
    private String type;

    public RecyclerViewAdapterPokemons(Context context, ArrayList<Pokemons> data, String type) {
    ...
    @Override
    public RecyclerViewAdapterPokemons.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = mInflater.inflate(R.layout.recyclerview_row_tipos_pokemon, parent, false);

        return new RecyclerViewAdapterPokemons.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(RecyclerViewAdapterPokemons.ViewHolder holder, int position) {
        Pokemons pokemons = mData.get(position);
        String nome = pokemons.getPokemons().getName().substring(0,1).toUpperCase() + pokemons.getPokemons().getName().substring(1);
    ...
    public Pokemons getItem(int id) {
        return mData.get(id);
    }

    public void setClickListener(RecyclerViewAdapterTiposPokemon.ItemClickListener itemClickListener) {
